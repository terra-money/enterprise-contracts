use crate::contract::{
    query_member_vote, PROPOSAL_OUTCOME_ABSTAIN, PROPOSAL_OUTCOME_NO, PROPOSAL_OUTCOME_YES,
};
...
use enterprise_protocol::api::MemberVoteParams;
...
#[test]
fn member_vote_on_non_existent_proposal_is_none() -> DaoResult<()> {
    let mut deps = mock_dependencies();
    let env = mock_env();
    let info = mock_info("sender", &[]);

...
pub const ATTESTATION_TEXT: Option<&str> = Some("my_attestation");
...
pub fn startup_default_dao(with_attestation: bool) -> App {
    let mut app = startup_with_versioning();

    let msg = create_standard_msg_new_dao(with_attestation).into_execute_msg();
...
pub fn create_standard_msg_new_dao(with_attestation: bool) -> CreateDaoMsg {
    CreateDaoMsg {
        ...
        attestation_text: if !with_attestation {
            None
        } else {
            ATTESTATION_TEXT.map(|v| v.to_string())
        },
    }
}
